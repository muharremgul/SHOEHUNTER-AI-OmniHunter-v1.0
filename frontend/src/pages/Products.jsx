...
import { Plus, LinkSimple, CircleNotch, Crosshair, Trash } from "@phosphor-icons/react";
...
export function productClassification(product) {
  return {
    category: CATEGORY_LABELS[product?.category] || "Sınıflandırılmadı",
    origins: (product?.tracking_origins || ["legacy"]).map((origin) => ORIGIN_LABELS[origin] || origin),
    radar: Boolean(product?.radar_watch_count || product?.tracking_origins?.includes("radar")),
  };
}
...
export default function Products() {
  ...
  const [manual, setManual] = useState({ name: "", brand: "", model: "", category: "shoes", url: "" });
  ...
  const visibleProducts = useMemo(() => products.filter((product) => {
    const status = product.stock_status || (product.in_stock ? "in_stock" : "unknown");
    if (filter === "all") return true;
    if (filter === "inactive") return product.active === false;
    if (filter === "error") return ["error", "not_found"].includes(status);
    return status === filter && product.active !== false;
  }), [products, filter]);

  const filters = [
    { id: "in_stock", label: "Stokta" },
    { id: "all", label: "Tümü" },
    { id: "out_of_stock", label: "Stok Bekleniyor" },
  ...
  const addManual = async () => {
    ...
    try {
      ...
      setManual({ name: "", brand: "", model: "", category: "shoes", url: "" });
      setShowManual(false);
      load();
    } catch (e) {
  ...
  return (
    <div className="space-y-6" data-testid="products-page">
      ...
      {showManual && (
        <div className="card p-5 animate-fadeUp space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            <select data-testid="manual-category-input" className="input-dark" value={manual.category} onChange={(e) => setManual({ ...manual, category: e.target.value })}>
      ...
      <div className="flex gap-1 overflow-x-auto border-b border-zinc-800 pb-2" role="tablist" aria-label="Ürün stok filtresi">
        {filters.map((item) => (
          <button
            key={item.id}
            type="button"
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center">
          <div className="text-zinc-400 font-heading text-lg">Bu görünümde ürün yok</div>
          <div className="text-zinc-500 text-sm mt-2">Stok bekleyen ürünler yeniden stoğa girince Stokta sekmesinde otomatik görünür.</div>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {visibleProducts.map((p, i) => (
            (() => {
              const stock = stockLabel(p.stock_status, p.in_stock);
              const classification = productClassification(p);
              return (
              <Link
              key={p.id}
              to={`/urunler/${p.id}`}
              data-testid={`product-card-${p.id}`}
              className={`card overflow-hidden hover:border-primary/40 hover:-translate-y-0.5 transition-all duration-200 animate-fadeUp group ${p.stock_status === 'out_of_stock' || p.active === false ? 'opacity-60 grayscale-[40%]' : ''}`}
            ...
            >
              <div className="h-40 bg-zinc-900 overflow-hidden relative">
                ...
                {classification.radar && (
                  <span className="absolute top-2 left-2 bg-primary text-black text-[10px] font-bold px-2 py-1 rounded flex items-center gap-1 shadow" data-testid={`radar-product-${p.id}`}>
                    <Crosshair size={12} weight="bold" /> RADAR İLE TAKİPTE
                  </span>
                )}
              ...
              <div className="p-4">
                ...
                <div className="flex flex-wrap gap-1 mt-2">
                  <span className="text-[10px] border border-zinc-700 rounded px-1.5 py-0.5 text-zinc-300">{classification.category}</span>
                  {classification.origins.map((origin) => (
                    <span key={origin} className="text-[10px] border border-zinc-800 rounded px-1.5 py-0.5 text-zinc-500">{origin}</span>
                  ))}
                  {p.radar_watch_count > 0 && (
                    <span className="text-[10px] text-primary px-1.5 py-0.5">{p.radar_watch_count} Radar</span>
                  )}
                </div>
