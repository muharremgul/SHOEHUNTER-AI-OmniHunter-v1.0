...
import { ArrowLeft, ArrowsClockwise, Trash, Plus, CircleNotch, Crosshair, Target, Brain, CheckCircle, Warning, PencilSimple, Palette } from "@phosphor-icons/react";
...
const ORIGIN_LABELS = {
  radar: "Ürün Radarı",
  link: "Link ile eklendi",
  ai_search: "AI Arama ile eklendi",
...
function BuyAdviceCard({ advice }) {
  ...
  return (
    <div className="card p-6 border-primary/20" data-testid="buy-advice-card">
      ...
      {(advice.total_cost || advice.seller_trust) && (
        <div className="mt-4 pt-3 border-t border-zinc-800 flex flex-wrap gap-x-6 gap-y-1 text-xs font-mono">
          {advice.total_cost && (
            <span className={advice.total_cost.complete ? "text-emerald-300" : "text-amber-300"}>
              Toplam maliyet: {fmtPrice(advice.total_cost.amount)} · {advice.total_cost.complete ? "doğrulandı" : "kargo/koşul eksik"}
            </span>
          )}
          {advice.seller_trust && (
            <span className={advice.seller_trust.band === "high" ? "text-emerald-300" : advice.seller_trust.band === "low" ? "text-red-300" : "text-amber-300"}>
              Satıcı kanıtı: {advice.seller_trust.score}/100
            </span>
          )}
        </div>
      )}
...
export default function ProductDetail() {
  ...
  const { product, listings, rules, alerts, history, family, watches = [] } = data;
  const productOrigins = product.tracking_origins || ["legacy"];
  const radarTracked = product.radar_watch_count > 0 || productOrigins.includes("radar");
    ...
    <div className="space-y-6" data-testid="product-detail-page">
      ...
      <div className="flex items-start gap-6 flex-wrap">
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-2 mt-3" data-testid="product-classification">
            <span className="text-xs border border-zinc-700 rounded px-2 py-1 text-zinc-300">
              Sınıf: {CATEGORY_LABELS[product.category] || "Sınıflandırılmadı"}
            </span>
            {productOrigins.map((origin) => (
              <span key={origin} className="text-xs border border-zinc-800 rounded px-2 py-1 text-zinc-500">
                {ORIGIN_LABELS[origin] || origin}
              </span>
            ))}
            {radarTracked && (
              <span className="text-xs bg-primary text-black font-bold rounded px-2 py-1 flex items-center gap-1" data-testid="radar-tracked-badge">
                <Crosshair size={13} weight="bold" /> RADAR İLE TAKİPTE · {watches.length || product.radar_watch_count} Radar
              </span>
            )}
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-zinc-500 text-xs uppercase tracking-wider border-b border-zinc-800">
                  ...
                  <th className="py-2 pr-4">Toplam maliyet</th>
                  <th className="py-2 pr-4">Satıcı güveni</th>
              ...
              <tbody>
                {listings.map((l) => {
                  ...
                  const cartDeal = Number.isFinite(Number(l.last_cart_price))
                    && Number.isFinite(Number(l.last_price))
                    && Number(l.last_cart_price) < Number(l.last_price)
                    && !(l.last_cart_price_conditions || []).length;
                  const cartCampaign = (l.campaigns || []).find((campaign) => campaign.type === "cart");
                  return (
                  <tr key={l.id} className="border-b border-zinc-800 hover:bg-zinc-900/50 last:border-0" data-testid={`listing-row-${l.id}`}>
                    <td className="py-3 pr-4">
                      ...
                      {!cartDeal && cartCampaign?.label && (
                        <div className="text-[11px] text-amber-300 mt-0.5 max-w-[260px] line-clamp-2">
                          Sepet kampanyası: {cartCampaign.label}
                        </div>
                      )}
                    </td>
                    <td className="py-3 pr-4 font-mono font-bold text-primary tabular-nums">
                      {fmtPrice(cartDeal ? l.last_cart_price : l.last_price)}
                      {cartDeal && <span className="badge-discount ml-2">Sepette</span>}
                    </td>
                    <td className="py-3 pr-4 font-mono text-xs tabular-nums">
                      <div className="text-white">{fmtPrice(l.total_cost?.amount)}</div>
                      <div className={l.total_cost?.complete ? "text-emerald-400" : "text-amber-300"}>
                        {l.total_cost?.complete ? "Doğrulandı" : "Eksik koşul/kargo"}
                      </div>
                    </td>
                    <td className="py-3 pr-4 text-xs">
                      <div className={l.seller_trust?.band === "high" ? "text-emerald-400" : l.seller_trust?.band === "low" ? "text-red-400" : "text-amber-300"}>
                        {Number.isFinite(Number(l.seller_trust?.score)) ? `${l.seller_trust.score}/100` : "—"}
                      </div>
                      <div className="text-zinc-500 max-w-[180px] truncate" title={l.seller_trust?.disclaimer}>
                        {l.seller || (l.seller_trust?.marketplace ? "Satıcı bilinmiyor" : "Doğrudan mağaza")}
                      </div>
                    </td>
                    <td className="py-3 pr-4 font-mono text-zinc-500 line-through tabular-nums">
                      {cartDeal ? fmtPrice(l.last_price) : (l.last_old_price ? fmtPrice(l.last_old_price) : "")}
                    </td>
