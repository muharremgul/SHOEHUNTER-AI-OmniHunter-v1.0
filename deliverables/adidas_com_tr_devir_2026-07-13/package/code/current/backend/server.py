...
async def apply_scheduler_settings():
    ...
    if sched.get("enabled"):
        ...
        discovery_hours = max(6, int(sched.get("discovery_interval_hours") or 12))
        scheduler.add_job(
            scheduled_discovery,
...
class WatchCreate(BaseModel):
    ...
    discovery_frequency_hours: int = Field(default=12, ge=6, le=168)
...
async def quick_track(body: QuickTrack):
    ...
    elif image and not product.get("image"):
        await db.products.update_one({"id": product["id"]}, {"$set": {"image": image}})
        product["image"] = image
    listing = {
        "id": new_id(),
...
async def metrics():
    ...
    for health in await health_snapshot(db):
        ...
        lines.append(
            f'shoehunter_store_circuit_open{{store="{slug}"}} ' f'{1 if health.get("circuit_state") == "open" else 0}'
        )
    return "\n".join(lines) + "\n"
...
async def zero_link_search(body: SearchRequest):
    ...
    async def _search(engine):
        try:
                ...
                results = [
                    {
                        ...
                        "url": item["url"],
                        "score": _score_result(item.get("title"), search_query),
                        "image": None,
                        "price": None,
                        "store": engine.name,
            ...
            await record_store_result(db, engine.slug, "timeout", parser_version=getattr(engine, "parser_version", "1"))
            return {
                "store": engine.name,
                "status": "timeout",
                "results": [],
            ...
            await record_store_result(
                db, engine.slug, final_status, parser_version=getattr(engine, "parser_version", "1")
            )
            return {
                "store": engine.name,
                "status": final_status,
            ...
            js_results = [
                {
                    ...
                    "status": "timeout",
                    "results": [],
                    "error": "Tarayıcı motoru zaman aşımı",
                    "engine": "playwright",
                }
                for e in js_engines
            ]

    for store_result, engine in zip(js_results, js_engines):
...
async def track_candidate(body: TrackCandidate):
    ...
    if existing:
        raise HTTPException(409, "Bu link zaten takip ediliyor")
    try:
        return await quick_track(QuickTrack(url=url, name=body.title, image=body.image))
...
async def create_watch(body: WatchCreate):
    ...
    watch = make_watch_document(body.model_dump(), analysis=analysis)
...
async def privacy_status():
    ...
    return {
        "providers": providers,
        "profile_fields": ["weight", "target_weight", "shoe_size", "foot_notes", "usage", "priorities", "notes"],
