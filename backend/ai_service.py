...
async def _save_messages(db, session_id, user_message, assistant_message):
    from services import new_id, now_iso

    await db.ai_messages.insert_one(
        {"id": new_id(), "session_id": session_id, "role": "user", "content": user_message, "created_at": now_iso()}
    )
    await db.ai_messages.insert_one(
        {
            "id": new_id(),
...
async def coach_stream(db, session_id, message):
    ...
    if GEMINI_KEY or GROQ_KEY or OPENAI_KEY:
        ...
        try:
            ...
            yield f"data: {json.dumps({'done': True})}\n\n"
        try:
            await _save_messages(db, session_id, message, full_text)
        except Exception:
            pass
        return
