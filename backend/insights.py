...
def compute_buy_decision(listing, rule, insight, profile, product_name=""):
    ...
    if listing.get("last_cart_price") and not (listing.get("last_cart_price_conditions") or []):
        cart_confidence = float(listing.get("last_cart_price_confidence") or 0)
        conf_score = max(conf_score, 15 if cart_confidence >= 0.9 else 10)
        reasons.append("Sepette fiyat görünür ürün teklifinden doğrulandı")
