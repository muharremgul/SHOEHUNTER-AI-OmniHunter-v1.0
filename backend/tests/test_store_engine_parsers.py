...
def test_score_result_rejects_brand_only_wrong_model_and_wrong_generation():
    assert _score_result("Nike", "Nike Zegama 2") == 0
    assert _score_result("Nike Initiator 2 Erkek Ayakkabi", "Nike Zegama 2") == 0
    assert _score_result("Nike Zegama Trail Erkek Ayakkabi", "Nike Zegama 2") == 0
    assert _score_result("Nike Zegama Trail 3 Erkek Ayakkabi", "Nike Zegama 2") == 0
    assert _score_result("Nike Zegama Trail 2 Erkek Ayakkabi", "Nike Zegama 2") >= 75
...
def test_decathlon_dkt_product_uses_only_selected_model_sizes_and_availability():
    html = """
    <html>
      <head>
        <meta property="og:title" content="Saved local title">
        <meta property="product:original_price:amount" content="2890">
      </head>
      <body>
        <script>
        __DKT = {
          "_ctx": {
            "data": [{
              ...
              "data": {
                "supermodelId": "340673",
                "webLabel": "Erkek Yelkenli Yağmurluğu - Sailing 500",
                "models": [{
                  "modelId": "8758966",
                  "webLabel": "Erkek Yelkenli Yağmurluğu - Lacivert - Sailing 500",
                  "url": "p/erkek-yelkenli-yagmurlugu-lacivert-sailing-500/_/R-p-340673?mc=8758966&c=MAVI",
                  "price": 2890,
                  "currency": "TRY",
                  "image": {"url": "https://contents.mediadecathlon.com/p2950468/picture.jpg"},
                  "skus": [
                    {"skuId": "4563342", "size": "S.", "price": 2890},
                    {"skuId": "4563343", "size": "M.", "price": 2890},
                    {"skuId": "4563346", "size": "2XL.", "price": 2890,
                     "isNotAvailable": true, "isNotAvailableOnline": true}
                  ]
                }, {
                  "modelId": "9999999",
                  "webLabel": "Erkek Yelkenli Yağmurluğu - Kırmızı - Sailing 500",
                  "url": "p/erkek-yelkenli-yagmurlugu-kirmizi-sailing-500/_/R-p-340673?mc=9999999&c=KIRMIZI",
                  "price": 2990,
                  "skus": [{"skuId": "999", "size": "XL."}]
                }]
              }
            }]
          }
        ...
        <script>{"skuId":"unrelated-recommendation","size":"47/50"}</script>
      </body>
    </html>
    """
    url = (
        "https://www.decathlon.com.tr/p/erkek-yelkenli-yagmurlugu-lacivert-sailing-500/"
        "_/R-p-340673?mc=8758966&c=MAVI"
    )

    result = engine("decathlon").parse(html, url)

    assert result["title"] == "Erkek Yelkenli Yağmurluğu - Lacivert - Sailing 500"
    assert result["current_price"] == 2890
    assert result["price_source"] == "decathlon_dkt:model.price"
    assert result["image"] == "https://contents.mediadecathlon.com/p2950468/picture.jpg"
    assert {item["name"]: item["in_stock"] for item in result["sizes"]} == {
        "S": True,
        "M": True,
        "2XL": False,
    }
    ...
    assert all(item["sku"] != "unrelated-recommendation" for item in result["sizes"])
    assert result["variant_urls"] == [
        "https://www.decathlon.com.tr/p/erkek-yelkenli-yagmurlugu-kirmizi-sailing-500/"
        "_/R-p-340673?mc=9999999&c=KIRMIZI"
    ]


def test_decathlon_search_reads_products_from_dkt_payload():
    html = """
    <script>
      __DKT = {"_ctx":{"data":[{"type":"SearchResults","data":{"products":[{
        "modelId":"8758966",
        "webLabel":"Erkek Yelkenli Yağmurluğu Lacivert Sailing 500",
        "url":"p/erkek-yelkenli-yagmurlugu-lacivert-sailing-500/_/R-p-340673?mc=8758966",
        "price":2890,
        "image":{"url":"https://contents.mediadecathlon.com/p2950468/picture.jpg"}
      }]}}]}};
    </script>
    ...
    results = engine("decathlon").parse_search(
        html,
        "Sailing 500",
        "https://www.decathlon.com.tr/search?Ntt=Sailing+500",
    )

    assert len(results) == 1
    assert results[0]["price"] == 2890
    assert results[0]["url"].endswith("R-p-340673?mc=8758966")
    assert results[0]["image"] == "https://contents.mediadecathlon.com/p2950468/picture.jpg"
