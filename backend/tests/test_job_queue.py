...
@pytest.mark.asyncio
async def test_expired_running_job_is_reclaimed_but_active_lease_is_not():
    client, db, name = await temporary_database()
    try:
        expired = await enqueue_job(db, "test", idempotency_key="expired-job")
        active = await enqueue_job(db, "test", idempotency_key="active-job")
        now = datetime.now(timezone.utc)
        await db.jobs.update_one(
            {"id": expired["id"]},
            {"$set": {"status": "running", "worker_id": "dead-worker", "lease_until": now - timedelta(seconds=1)}},
        )
        await db.jobs.update_one(
            {"id": active["id"]},
            {"$set": {"status": "running", "worker_id": "live-worker", "lease_until": now + timedelta(minutes=5)}},
        )

        reclaimed = await claim_job(db, "replacement-worker")
        assert reclaimed["id"] == expired["id"]
        assert reclaimed["worker_id"] == "replacement-worker"
        assert await claim_job(db, "other-worker") is None
    finally:
        await client.drop_database(name)
        client.close()
