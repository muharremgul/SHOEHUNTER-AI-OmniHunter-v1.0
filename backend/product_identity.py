...
def identity_from_title(title, brand=None, url=None, category=None):
    ...
    normalized_source = " ".join(tokens)
    detected_brand = next(
        (canonical for alias, canonical in BRAND_ALIASES if alias in normalized_source),
        None,
    )
    brand = brand or detected_brand
    style_codes = [m.group(0).upper() for m in STYLE_CODE_RE.finditer(str(title or ""))]
    if url:
        style_codes.extend(
            match.group(0).upper()
            for match in STYLE_CODE_RE.finditer(str(url))
...
def match_identities(expected, candidate, required_tokens=None, excluded_tokens=None):
    ...
    if exact_codes:
        ...
        expected_codes = {
            code for code in expected.model_codes if any(char.isalpha() for char in code)
        }
        candidate_codes = {
            code for code in candidate.model_codes if any(char.isalpha() for char in code)
        }
        if expected_codes and candidate_codes and expected_codes.isdisjoint(candidate_codes):
            return {
                "confidence": 0.0,
                "decision": "rejected",
                "evidence": ["model_code_mismatch"],
            }
        if expected.brand and candidate.brand and expected.brand != candidate.brand:
            return {
                "confidence": 0.0,
                "decision": "rejected",
                "evidence": ["brand_mismatch"],
            }
        if expected.generation and not candidate.generation:
            return {
                "confidence": 0.0,
                "decision": "rejected",
                "evidence": ["generation_missing"],
            }
        expected_core = set(expected.model_tokens) - {expected.generation}
        candidate_core = set(candidate.model_tokens) - {candidate.generation}
        if expected_core and not (expected_core & candidate_core):
            return {
                "confidence": 0.0,
                "decision": "rejected",
                "evidence": ["model_token_mismatch"],
            }
...
def canonicalize_product_url(url):
    ...
    if hostname == "amazon.com.tr" or hostname.endswith(".amazon.com.tr"):
        asin_match = re.search(r"/(?:dp|gp/product)/([A-Z0-9]{10})(?:/|$)", parsed.path, re.I)
        if asin_match:
            return f"https://www.amazon.com.tr/dp/{asin_match.group(1).upper()}"
